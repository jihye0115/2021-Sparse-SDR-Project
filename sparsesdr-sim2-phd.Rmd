...
niter <- 100
p.sim2 <- c()
sphd.cor1 <- c(); sphd.cor2 <- c()
sphd.mse1 <- c(); sphd.mse2 <- c()

for (i in 1:niter){
  y2.100 = cos(2*(t(beta.true[,1])%*%t(x1))) - cos(t(beta.true[,2])%*%t(x1)) + sigma*rnorm(n1)
  lambda1 = c(0.2,0.2)
  ssdr.result1 <- ssdr.lambda(x1, y2.100, method="phdres", d=2, nslices=5, 
                              lambda1, lambda2=0.01, max.iter=200, eps.conv=0.01)

  
  beta <- beta.order(ssdr.result1$beta)

  # number of p
  p.sim2 <- c(p.sim2, ssdr.result1$p.e)

  # corr with true beta
  sphd.cor1 <- c(sphd.cor1, abs(cor(as.vector(beta[,1]%*%t(x1)),
                                    as.vector(t(beta.true.norm[,1])%*%t(x1)) )))
  sphd.cor2 <- c(sphd.cor2, abs(cor(as.vector(beta[,2]%*%t(x1)),
                                    as.vector(t(beta.true.norm[,2])%*%t(x1)))))

  # mse with true x
  sphd.mse1 <- c(sphd.mse1, mean((as.vector(beta[,1]%*%t(x1)) -
                                    as.vector(beta.true.norm[,1]%*%t(x1)))^2))
  sphd.mse2 <- c(sphd.mse2, mean((as.vector(beta[,2]%*%t(x1)) -
                                    as.vector(beta.true.norm[,2]%*%t(x1)))^2))
}

mean(sphd.cor1, na.rm=TRUE)
mean(sphd.cor2, na.rm=TRUE)
...
niter <- 100
phd.cor1 <- c(); phd.cor2 <- c()
phd.mse1 <- c(); phd.mse2 <- c()

for (i in 1:niter){

  y2.100 = cos(2*(t(beta.true[,1])%*%t(x1))) - cos(t(beta.true[,2])%*%t(x1)) + sigma*rnorm(n1)
  out = comp.sdr.phdres(x1,y2.100,d=2)
  beta = out$beta.sdr
  beta <- beta.order(beta)

  # corr with true beta
  phd.cor1 <- c(phd.cor1, abs(cor(as.vector(beta[,1]%*%t(x1)),as.vector(t(beta.true.norm[,1])%*%t(x1)))))
  phd.cor2 <- c(phd.cor2, abs(cor(as.vector(beta[,2]%*%t(x1)),as.vector(t(beta.true.norm[,2])%*%t(x1)))))
  
  # mse with true x
  phd.mse1 <- c(phd.mse1, mean((as.vector(beta[,1]%*%t(x1)) - as.vector(beta.true.norm[,1]%*%t(x1)))^2))
  phd.mse2 <- c(phd.mse2, mean((as.vector(beta[,2]%*%t(x1))- as.vector(beta.true.norm[,2]%*%t(x1)))^2))
}
...
for (i in 1:niter){
  y2.200 = cos(2*(t(beta.true[,1])%*%t(x2))) - cos(t(beta.true[,2])%*%t(x2)) + sigma*rnorm(n2)
  lambda1 = c(0.06,0.06)
  ssdr.result1 <- ssdr.lambda(x2, y2.200, method="phdres", d=2, nslices=7, 
                              lambda1, lambda2=0.01, max.iter=200, eps.conv=0.001)
  ...
  # corr with true beta
  sphd.cor1 <- c(sphd.cor1, abs(cor(as.vector(beta[,1]%*%t(x2)),
                                    as.vector(t(beta.true.norm[,1])%*%t(x2)) )))
  sphd.cor2 <- c(sphd.cor2, abs(cor(as.vector(beta[,2]%*%t(x2)),
                                    as.vector(t(beta.true.norm[,2])%*%t(x2)))))

  # mse with true x
  sphd.mse1 <- c(sphd.mse1, mean((as.vector(beta[,1]%*%t(x1)) -
                                    as.vector(beta.true.norm[,1]%*%t(x2)))^2))
  sphd.mse2 <- c(sphd.mse2, mean((as.vector(beta[,2]%*%t(x2)) -
                                    as.vector(beta.true.norm[,2]%*%t(x2)))^2))
}
...
for (i in 1:niter){
  y2.200 = cos(2*(t(beta.true[,1])%*%t(x2))) - cos(t(beta.true[,2])%*%t(x2)) + sigma*rnorm(n2)
  out = comp.sdr.phdres(x2,y2.200,d=2)
  beta = out$beta.sdr
  beta <- beta.order(beta)

  # corr with true beta
  phd.cor1 <- c(phd.cor1, abs(cor(as.vector(beta[,1]%*%t(x2)),as.vector(t(beta.true.norm[,1])%*%t(x2)))))
  phd.cor2 <- c(phd.cor2, abs(cor(as.vector(beta[,2]%*%t(x2)),as.vector(t(beta.true.norm[,2])%*%t(x2)))))
  
  # mse with true x
  phd.mse1 <- c(phd.mse1, mean((as.vector(beta[,1]%*%t(x2)) - as.vector(beta.true.norm[,1]%*%t(x2)))^2))
  phd.mse2 <- c(phd.mse2, mean((as.vector(beta[,2]%*%t(x2)) - as.vector(beta.true.norm[,2]%*%t(x2)))^2))
}
