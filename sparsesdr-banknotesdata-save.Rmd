...
head(banknote)
X <- banknote %>% dplyr::select(-Status); dim(X)
yy = ifelse (banknote$Status == 'counterfeit', 1,0)
pairs(X)


lambda1 = c(0.01,0.01)
ssdr.result1 <- ssdr.lambda(X, yy, method="save", d=2, nslices=2, lambda1, lambda2=0.001, max.iter=200, eps.conv=1e-3)
ssdr.result1


label = c("genuine", "counterfeit")
print.data = data.frame(ssave=t(t(ssdr.result1$beta)%*%t(X)), save=t(t(ssdr.result1$beta0)%*%t(X)), y=yy)


ggplot(print.data) + geom_point(aes(ssave.1,ssave.2, col=factor(y))) +  
  ggtitle("Sparse SAVE with Swiss banknote data") +
  scale_color_discrete(name="Status", 
                       breaks=c(0,1), 
                       labels=label) + 
  xlab('S-SAVE 1st direction') +
  ylab('S-SAVE 2nd direction')


ggplot(print.data) + geom_point(aes(save.1,save.2, col=factor(y)))+
  ggtitle("SAVE with Swiss banknote data") +
  scale_color_discrete(name="Status", 
                       breaks=c(0,1), 
                       labels=label) + 
  xlab('SAVE 1st direction') +
  ylab('SAVE 2nd direction')



cor(print.data$ssave.1, print.data$save.1)
cor(print.data$ssave.2, print.data$save.2)
